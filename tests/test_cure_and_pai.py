...
def test_pretraining_alignment_index():
    ...
    batch1 = EpisodeBatch(
        episodes=[ep1], dataset_name="sim", format="hdf5", source_path="/tmp/sim.h5"
    )
    batch2 = EpisodeBatch(
        episodes=[ep2], dataset_name="real", format="hdf5", source_path="/tmp/real.h5"
    )
...
def test_latent_embeddings_clip_fallback():
    ep = _make_mock_episode()
    batch = EpisodeBatch(
        episodes=[ep], dataset_name="test", format="hdf5", source_path="/tmp/test.h5"
    )
