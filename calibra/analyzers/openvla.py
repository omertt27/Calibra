...
class OpenVLACompatibilityAnalyzer(Analyzer):
    ...
    def analyze(
        ...
        all_actions = (
            np.concatenate([ep.actions for ep in batch.episodes if ep.actions.ndim > 1], axis=0)
            if batch.episodes
            else None
        )
        ...
        if n_critical == 0 and n_warning == 0:
            ...
            compatible = False
            explanation = (
                f"Dataset has {n_critical} critical issue(s) that block OpenVLA fine-tuning."
            )
