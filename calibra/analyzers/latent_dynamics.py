...
class LatentDynamicsAnalyzer(Analyzer):
    ...
    """
    proprio_keys: tuple[str, ...] = ("proprio", "state", "joint_state", "joint_pos",
    ...
    def analyze(
        ...
        state_redundancy = float(1.0 - (unique_state_voxels / len(all_states))) if len(all_states) > 0 else 0.0
        ...
        trans_redundancy = float(1.0 - (len(unique_trans_voxels_set) / total_trans)) if total_trans > 0 else 0.0
        ...
        for ep_id, count in episode_exclusive_counts.items():
            episode_exclusive_novelty[ep_id] = float(count / total_unique_voxels) if total_unique_voxels > 0 else 0.0
        ...
        if trans_redundancy > 0.8:
            ...
            red_implication = "Transition redundancy is low; most demonstrations contain novel dynamics."
        ...
        if action_effect_mi < 0.1:
            ...
            mi_implication = "Causal dependency between robot actions and physical transitions is critically low. Actions may be decoupled from environment physics (e.g. control noise or actuator slips)."
        else:
            mi_level = RiskLevel.OK
            mi_implication = "Actions exhibit healthy causal influence on state transitions."
