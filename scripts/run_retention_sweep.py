...
def generate_task_dataset(
    task_name: str, n_eps: int, n_steps: int, rng_seed: int, noise_scale: float
) -> EpisodeBatch:
    ...
    for i in range(n_eps):
        ...
        for t in range(n_steps - 1):
            states[t + 1] = dyn_fn(states[t : t + 1], actions[t : t + 1])[0] + rng.normal(
                0, noise_scale, 2
            )
...
def train_and_eval_task(
    batch: EpisodeBatch, task_name: str, X_test: np.ndarray, Y_test: np.ndarray
) -> float:
    states_list = []
    actions_list = []
    next_states_list = []
...
def main():
        ...
        for ratio in ratios:
            ...
            for seed in range(3):  # 3 seeds for speed
                ...
                random_batch = EpisodeBatch(
                    shuffled[:k_target], f"rand_{seed}", "hdf5", f"rand_{seed}"
                )
            ...
            div_eps = [
                e for e in pool_episodes if e.metadata.episode_id in div_res.keep_episode_ids
            ]
            ...
            nov_eps = [
                e for e in pool_episodes if e.metadata.episode_id in nov_res.keep_episode_ids
            ]
            nov_batch = EpisodeBatch(nov_eps[:k_target], "nov", "hdf5", "nov")
            n_perf = train_and_eval_task(nov_batch, task, X_test, Y_test)

            # Save results
            results[task][ratio] = {"random": r_perf, "diversity": d_perf, "novelty": n_perf}
            print(f"    - Random: {r_perf:.4f} | Diversity: {d_perf:.4f} | Novelty: {n_perf:.4f}")

    # ── 6. Generate Markdown Artifact ───────────────────────────────────────
    artifact_path = Path(
        "/Users/omer/.gemini/antigravity-cli/brain/756f676d-5cec-4126-9311-8d2fb3a9b0af/robomimic_retention_sweep.md"
    )
    ...
    for task in tasks:
        ...
        for r in ratios:
            ...
            rand_ret = res["random"]
            rows.append(f"| {r:.0%} | {rand_ret:.4f} | {div_ret:.4f} | **{nov_ret:.4f}** |")
