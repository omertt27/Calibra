...
def generate_system_overview() -> None:
    ...
    def box(
        x, y, w, h, label, sublabel="", color="#E3F2FD", textcolor="#1A237E", fontsize=10, bold=True
    ):
        rect = FancyBboxPatch(
            (x, y),
            w,
    ...
    def arrow(x1, y1, x2, y2):
        ax.annotate(
            ...
            xy=(x2, y2),
            xytext=(x1, y1),
            arrowprops=dict(arrowstyle="-|>", color="#455A64", lw=1.5, mutation_scale=15),
        )

    def label(x, y, text, fontsize=8.5, color="#455A64"):
        ax.text(
            x, y, text, ha="center", va="center", fontsize=fontsize, color=color, style="italic"
        )
    ...
    box(
        ...
        7.5,
        0.55,
        "Format Adapters  —  metadata-first, lazy image skip",
        color="#F3E5F5",
        textcolor="#4A148C",
        fontsize=9,
    )
    ...
    for i, (name, sub) in enumerate(analyzers):
        x = 0.3 + i * 1.9
        box(x, 2.1, 1.7, 0.6, name, sub, color="#E3F2FD", textcolor="#0D47A1", fontsize=8.5)
    ...
    box(
        ...
        7.5,
        0.4,
        "DiagnosticReport  —  metric values · risk flags · bootstrap CIs",
        color="#FBE9E7",
        textcolor="#BF360C",
        fontsize=9,
    )
