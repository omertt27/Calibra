...
def generate_mh_dataset(n_eps: int = 15, n_steps: int = 100) -> EpisodeBatch:
    ...
    for i in range(n_eps):
        ...
        if path_type == 0:
            ...
            states_ideal = np.stack(
                [np.linspace(-1.5, 1.5, n_steps), np.linspace(1.5, -1.5, n_steps)], axis=1
            )  # Diagonal
...
def generate_mg_dataset(n_eps: int = 100, n_steps: int = 100) -> EpisodeBatch:
    ...
    for i in range(n_eps):
        ...
        for t in range(n_steps - 1):
            ...
            states[t + 1] = true_dynamics(states[t : t + 1], actions[t : t + 1])[0] + rng.normal(
                0, 0.15, 2
            )
...
def main():
    ...
    artifact_path = Path(
        "/Users/omer/.gemini/antigravity-cli/brain/756f676d-5cec-4126-9311-8d2fb3a9b0af/robomimic_experiment_results.md"
    )
