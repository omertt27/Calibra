...
rate (vs.\ 62\% random baseline at the same compute budget) establish Calibra
as a practical tool for robotics labs. We release Calibra as open-source
software with 377 tests, public PyPI package (\texttt{calibra-robotics}), and
a growing library of reference profiles.

\paragraph{Future work.}
Key directions include: (1) learning diagnostic thresholds directly from
accumulated training outcomes at scale; (2) extending real-time feedback to
mid-episode streaming (e.g., alerting during a bad grasp before the episode
ends); (3) integration with HuggingFace dataset cards to surface quality
metrics directly on the Hub.
