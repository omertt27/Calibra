...
def _scripted_expert(
    obs: np.ndarray, rng: np.random.Generator, noise_std: float = 6.0
) -> np.ndarray:
    agent_pos = obs[:2]
    block_pos = obs[2:4]
...
def collect_base_data(n_episodes: int = N_COLLECT, seed: int = SEED) -> EpisodeBatch:
    ...
    return EpisodeBatch(
        episodes=episodes, dataset_name="pusht_base", format="gym", source_path="gym_pusht/PushT-v0"
    )
...
def select_calibra_coreset(batch: EpisodeBatch, keep: float = KEEP_FRACTION) -> list[int]:
...
def _corrupt_spike(
    episodes: list[Episode], fraction: float, rng: np.random.Generator
) -> list[Episode]:
...
def train_bc(batch: EpisodeBatch) -> tuple[_BCPolicy, float]:
    ...
    loss_fn = nn.MSELoss()
    loader = DataLoader(
        TensorDataset(torch.from_numpy(states).to(DEVICE), torch.from_numpy(actions).to(DEVICE)),
...
def print_results(rows: list[dict], metrics: dict) -> None:
    ...
    print(
        f"  Spearman ρ : {m['spearman_rho']:.4f}  (p={m['spearman_p']:.4g})"
        f"  {'✅' if m['spearman_rho'] > 0.65 else '⚠️  target >0.65'}"
    )
    print(_THIN)
    print("  L4 — FAILURE PREDICTION")
    print(
        f"  Accuracy        : {m['accuracy']:.1%}   {'✅' if m['accuracy'] >= 0.70 else '❌'}  (target ≥70%)"
    )
    ...
    print(
        f"  Root-cause acc : {m['root_cause_accuracy']:.1%}"
...
def save_figures(rows: list[dict], metrics: dict) -> None:
    ...
    ax.set_title(
        f"L6 — Predicted vs Actual\nSpearman ρ = {rho:.3f}", fontsize=11, fontweight="bold"
    )
...
def main() -> None:
    ...
    parser.add_argument("--out-json", metavar="PATH", help="Save raw results to JSON")
    ...
    for i, (name, ctype, level, description) in enumerate(CONDITIONS):
        ...
        print(
            f"           predict -> score={pred['predicted_score']:.1f}  "
