...
def paired_ttest_p(a: list[float], b: list[float]) -> float:
    ...
    from math import lgamma, sqrt, pi, exp
    df = n - 1
    x = df / (df + t * t)
...
def density_split(episodes, tail_fraction: float = 0.15):
    ...
    common_ids = {ep.metadata.episode_id for ep in episodes
                  if ep.metadata.episode_id not in tail_ids}
...
def select_diversity(pool_batch, diag, frac: float, quality: bool):
    ...
    if quality:
        ...
        selector = CoresetSelector(
            keep_fraction=frac, strategy="diversity",
            max_spike_rate=1.0, max_vel_disc_rate=1.0,
            max_dropout_fraction=1.0, min_ldlj=-1000.0,
        )
...
def run_targeted_benchmark(
    ...
    train_batch = EpisodeBatch(
        episodes=train_eps, dataset_name=dataset_id,
        format="lerobot-v2", source_path=dataset_id,
    )
    ...
    _qselector = _CS(keep_fraction=1.0, strategy="diversity",
                     max_spike_rate=0.10, max_vel_disc_rate=0.25,
                     max_dropout_fraction=0.10, min_ldlj=-30.0)
    ...
    quality_batch = EpisodeBatch(
        episodes=quality_eps, dataset_name=dataset_id,
        format="lerobot-v2", source_path=dataset_id,
    )

    print(f"  Quality-approved pool: {len(quality_eps)}/{len(train_eps)} ep "
    ...
    print(f"  MSE overall={full_mse_overall:.4f}  common={full_mse_common:.4f}  "
    ...
    print(f"  MSE overall={qual_mse_overall:.4f}  common={qual_mse_common:.4f}  "
    ...
    det_selections: dict[str, dict[float, list]] = {
    ...
    for frac in fractions:
        ...
        tail_cov = {
            "quality_only":   sum(1 for ep in qualonly_eps if ep.metadata.episode_id in train_tail_ids) / n_train_tail,
            "diversity_only": sum(1 for ep in divonly_eps  if ep.metadata.episode_id in train_tail_ids) / n_train_tail,
            "calibra":        sum(1 for ep in calibra_eps  if ep.metadata.episode_id in train_tail_ids) / n_train_tail,
        }

        print(f"  Calibra selected {len(calibra_eps)} ep  |  "
              f"diversity_only {len(divonly_eps)} ep  |  quality_only {len(qualonly_eps)} ep")
        print(f"  Tail coverage: calibra={tail_cov['calibra']:.1%}  "
              f"div_only={tail_cov['diversity_only']:.1%}  "
              f"qual_only={tail_cov['quality_only']:.1%}")

        method_runs: dict[str, dict[str, list[float]]] = {
            m: {"overall": [], "common": [], "tail": [], "train_s": []}
            for m in METHODS
        }
        ...
        for seed in range(n_seeds):
            ...
            method_runs["random_full"]["common"].append(evaluate_bc(m, test_common_eps) if test_common_eps else float("nan"))
            method_runs["random_full"]["tail"].append(evaluate_bc(m, test_tail_eps) if test_tail_eps else float("nan"))
            ...
            method_runs["random_quality"]["common"].append(evaluate_bc(m, test_common_eps) if test_common_eps else float("nan"))
            method_runs["random_quality"]["tail"].append(evaluate_bc(m, test_tail_eps) if test_tail_eps else float("nan"))
            ...
            for label, eps in [
                ...
                method_runs[label]["common"].append(evaluate_bc(m, test_common_eps) if test_common_eps else float("nan"))
                method_runs[label]["tail"].append(evaluate_bc(m, test_tail_eps) if test_tail_eps else float("nan"))
            print("  det=done")
            ...
            if method == "random_full":
                ...
                for seed in range(n_seeds):
                    ...
                    rnd_tail_covs.append(
                        sum(1 for ep in rnd_eps_s if ep.metadata.episode_id in train_tail_ids) / n_train_tail
                    )
    ...
    print(f"  Full unfiltered: overall={full_mse_overall:.2f}  common={full_mse_common:.2f}  tail={full_mse_tail:.2f}")
    print(f"  Quality-approved full: overall={qual_mse_overall:.2f}  common={qual_mse_common:.2f}  tail={qual_mse_tail:.2f}")
    print(f"  Quality pool: {len(quality_eps)}/{len(train_eps)} ep  |  "
    ...
    for frac in fractions:
        ...
        print(f"  {'Method':<18}  {'TailCov':>8}  {'Overall':>9}  {'+-CI95':>7}  "
              f"{'DiffRef':>8}  {'p':>6}  {'d':>6}  {'Tail MSE':>9}  {'Common':>9}")
        print(f"  {'-'*18}  {'-'*8}  {'-'*9}  {'-'*7}  {'-'*8}  {'-'*6}  {'-'*6}  {'-'*9}  {'-'*9}")
        for method in METHODS:
            m = fr["methods"][method]
            ov  = m["overall"]
            tl  = m["tail"]
    ...
    try:
        ...
        for i, (method, (color, ls, label)) in enumerate(METHOD_STYLE.items()):
            ...
            offset = (i - 2) * width
            ax_ov.bar(x + offset, means_ov, width, label=label, color=color, alpha=0.8,
                      yerr=ci_ov, capsize=3)
            ax_com.bar(x + offset, means_co, width, label=label, color=color, alpha=0.8)
            ax_tl.bar(x + offset, means_tl, width, label=label, color=color, alpha=0.8,
                      yerr=ci_tl, capsize=3)
        ...
        for ax, title, ylabel in [
            ...
            ax.axhline(full_mse_overall, color="#6b7280", linewidth=1.2, linestyle=":",
                       label="Full unfiltered")
            ax.set_xticks(x); ax.set_xticklabels(xs)
            ax.set_xlabel("Retention fraction"); ax.set_ylabel(ylabel)
            ax.set_title(title, fontsize=11); ax.legend(fontsize=8); ax.grid(True, alpha=0.3)

        ax_tc.axhline(tail_fraction * 100, color="#6b7280", linewidth=1.2, linestyle=":",
        ...
        ax_tc.set_ylim(0, 105); ax_tc.legend(fontsize=8); ax_tc.grid(True, alpha=0.3)
...
if __name__ == "__main__":
    ...
    p.add_argument("--dataset", default="lerobot/pusht",
    ...
    p.add_argument("--n-epochs", type=int, default=120,
                   help="BC training epochs (default: 120)")
    p.add_argument("--n-seeds", type=int, default=10,
                   help="Training/selection seeds per method (default: 10)")
