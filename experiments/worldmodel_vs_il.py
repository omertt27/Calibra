...
def build_full_batch(goal: np.ndarray, n_episodes: int = 100):
    ...
    for i in range(n_clean):
        ...
        episodes.append(Episode(
            ...
            timestamps=ts, observations={"proprio": s}, actions=a,
        ))
    for i in range(n_redundant):
        s, a, ts = _generate_episode(n_clean + i, goal, CORRUPTION_RATE,
    ...
    return EpisodeBatch(
        episodes=episodes, dataset_name="pointmass_corrupted",
        source_path="synthetic", format="synthetic",
    )
...
def build_coreset_batch(full_batch, keep_fraction: float = 0.30):
    ...
    kept_eps = [ep for ep in full_batch.episodes
                if ep.metadata.episode_id in kept_ids]
...
def train_bc(batch, n_epochs: int = 120, lr: float = 1e-3):
    ...
    net = nn.Sequential(
        nn.Linear(4, 256), nn.LayerNorm(256), nn.ReLU(),
...
def evaluate_bc(net, s_mean, s_std, a_mean, a_std, device,
                goal, starts: list[np.ndarray]) -> float:
    import torch
    successes = 0
    with torch.no_grad():
        for start in starts:
...
class JEPAMPCController:
    ...
    def __init__(
        ...
        self.goal_latent = jepa.encode(
            np.array([goal[0], goal[1], 0.0, 0.0], dtype=np.float32)
        )
...
def evaluate_jepa_mpc(
    ...
    controller = JEPAMPCController(
        jepa, goal, horizon=horizon, n_samples=n_samples
    )
    successes = 0
    for start in starts:
        env = PointMassEnv(goal=goal, seed=42)
...
def run_worldmodel_vs_il():
    ...
    print(f"      Mean surprise: {mean_surprise:.3f}  |  High-surprise fraction: {high_surprise:.1%}")
...
def _save_comparison_figure(
    ...
    try:
        ...
        w = 0.35
        bars_id = ax1.bar(x - w / 2, id_scores, w, label="In-distribution",
                          color="#16a34a", alpha=0.85)
        bars_ood = ax1.bar(x + w / 2, ood_scores, w, label="Out-of-distribution",
        ...
        ax2.axvline(0.7, color="#dc2626", linestyle="--", linewidth=1.5,
                    label="High-surprise threshold")
