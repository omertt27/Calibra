...
def generate_benchmark_dataset(n_episodes=100):
    ...
    for i in range(n_redundant):
        ...
        states, actions, ts = generate_trajectory(
            seed=100 + i, is_redundant=True, redundancy_ref_seed=ref_seed
        )
        episodes.append(
            Episode(
...
def main():
    ...
    for frac in keep_fractions:
        ...
        selector = CoresetSelector(
            keep_fraction=frac, max_spike_rate=0.03, max_vel_disc_rate=0.90, min_ldlj=-25.0
        )
        pruned = selector.select(batch, report)
        keep_eps_calibra = [
            ep for ep in batch.episodes if ep.metadata.episode_id in pruned.keep_episode_ids
        ]
        ...
        print(
            f"Calibra {frac * 100:.0f}% -> {len(keep_eps_calibra)} episodes | Success: {success_c:.1f}%, Time: {t_c:.3f}s"
        )
        ...
        print(
            f"Random {frac * 100:.0f}% -> {len(keep_eps_random)} episodes | Success: {success_r:.1f}%, Time: {t_r:.3f}s"
        )
    ...
    ax.plot(
        np.array(fractions_plot) * 100,
        calibra_succs,
        marker="o",
        color="#2196F3",
        linewidth=2.5,
        label="Calibra Curation",
    )
    ax.plot(
        np.array(fractions_plot) * 100,
        random_succs,
    ...
    ax.set_title(
        "Coreset Pruning Performance: Calibra vs. Random Curation",
        fontsize=13,
        fontweight="bold",
        pad=15,
    )
